...
def requires_permission(user_msg: str, intent: dict) -> bool:
    ...
    # Map/chart generation needs permission
    return intent.get("needs_execution", False) or intent.get("is_map", False)


def is_confirmation(msg: str) -> bool:
    msg = msg.lower().strip().rstrip("!.,")
    return msg in CONFIRM_WORDS or any(w in msg for w in CONFIRM_WORDS)


def is_denial(msg: str) -> bool:
    msg = msg.lower().strip().rstrip("!.,")
    return msg in DENY_WORDS or any(w in msg for w in DENY_WORDS)
...
def permission_prompt(action_type: str, params: dict) -> str:
    ...
    if action_type == "map":
        dc    = params.get("district_col","district")
        vc    = params.get("value_col","value")
        title = params.get("title","")
        csv   = os.path.basename(params.get("csv_path",""))
        return (
            f"**Permission Required**\n\n"
            f"I need to run the following external tools to generate your visualization:\n\n"
            f"- **Python** (pandas, geopandas, matplotlib)\n"
            f"- **Data:** `{csv}`\n"
            f"- **Column:** `{dc}` → `{vc}`\n"
            f"- **Title:** {title}\n\n"
            f"This will execute code on your local machine.\n\n"
