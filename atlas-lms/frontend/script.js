...
function copyMsg(btn) {
  const t = btn.closest(".msg-body").querySelector(".msg-text");
  navigator.clipboard.writeText(t.innerText || t.textContent)
    .then(() => toast("Copied!"));
}
...
async function loadMapHistory() {
  try {
    ...
    list.innerHTML = hist.map(h => `
      <div class="mhc" onclick="viewHistMap('${h.map_file}','${h.id}','${h.title}')">
        <img src="${BACKEND}/storage/history/${h.map_file}" onerror="this.style.display='none'"/>
        <div class="mhc-info">
          <div class="mhc-title">${h.title || "Untitled"}</div>
          <div class="mhc-meta">${h.colormap} · ${h.timestamp}</div>
        </div>
      </div>`).join("");
  } catch {}
}

function viewHistMap(file, id, title) {
  currentMapId = id; currentTitle = title;
  if (!hasMessages) {
    document.getElementById("welcome").style.display  = "none";
    document.getElementById("messages").style.display = "block";
    hasMessages = true;
  }
  appendMsgWithMap(`Loaded from history: **${title}**`, `/storage/history/${file}`);
  switchView("chat", document.querySelector(".nav-item"));
}
...
async function loadMapsView() {
  try {
    ...
    if (!hist.length) {
      grid.innerHTML = `<div class="empty-state">No maps generated yet. Go to Chat and ask to generate a choropleth map!</div>`;
      return;
    }
    grid.innerHTML = hist.map(h => `
      <div class="map-card" onclick="viewHistMap('${h.map_file}','${h.id}','${encodeURIComponent(h.title)}')">
        <img src="${BACKEND}/storage/history/${h.map_file}" onerror="this.src=''"/>
        <div class="mc-info">
          <div class="mc-title">${h.title || "Untitled"}</div>
          <div class="mc-meta">${h.colormap} · ${h.value_col} · ${h.timestamp}</div>
        </div>
      </div>`).join("");
  } catch {}
}
