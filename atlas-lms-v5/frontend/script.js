...
async function sendMsg() {
  ...
  try {
    ...
    // Dataset auto-detection notice
    if (data.dataset_notice) {
      appendDatasetNotice(data.dataset_notice, data.dataset_meta);
    }
    // Update dataset badge in topbar
    if (data.has_dataset) {
      updateDatasetBadge(data.dataset_meta);
    }
...
// ─────────────────────────────────────────────────────────
let activeUploadedFile = null;  // { id, name, ext, file_type, path, result }

async function uploadFileMultimodal(event) {
  const file = event.target.files[0];
  if (!file) return;
  event.target.value = "";   // reset so same file can be re-selected

  const fub  = document.getElementById("fileUploadBadge");
  const name = document.getElementById("fileUploadName");
  fub.style.display = ""; name.textContent = `⏳ ${file.name}`;

  const fd = new FormData();
  fd.append("file",       file);
  fd.append("session_id", sessionId);
  fd.append("project_id", currentProjId || "");

  try {
    const res  = await fetch(`${BACKEND}/api/files/upload-and-process`, { method:"POST", body:fd });
    const data = await res.json();
    ...
    activeUploadedFile = data;
    name.textContent = `📎 ${file.name}`;

    // Show dataset badge if geo data detected
    if (data.has_dataset && data.dataset_meta) {
      updateDatasetBadge(data.dataset_meta);
    }

    // Auto-show file analysis in chat
    const fileEmoji = {
      image:"🖼️", pdf:"📄", excel:"📊", docx:"📝", text:"📃"
    }[data.file_type] || "📎";

    let summary = `${fileEmoji} **${file.name}** uploaded and processed.`;
    if (data.file_type === "image" && data.text) {
      summary += "\n\n" + data.text;
    } else if (data.preview) {
      summary += `\n\nPreview:\n\`\`\`\n${data.preview.slice(0,300)}\n\`\`\``;
    }
    if (data.dataset_notice) summary += "\n\n" + data.dataset_notice;
    if (data.pages)          summary += `\n\n📄 ${data.pages} pages extracted.`;
    if (data.sheets)         summary += `\n\n📊 ${data.sheets.length} sheet(s) found.`;
...
function clearFileUpload() {
  ...
  document.getElementById("fileUploadBadge").style.display = "none";
