"""
utils/execution_state.py
Per-request execution state for Atlas map generation.

Conversation state is intentionally separate from execution state. Pending map
requests store only request-scoped file paths and metadata; completed maps keep
...
def create_map_request(session_id: str, user_msg: str, dataset: dict,
    ...
    if source_csv and os.path.exists(source_csv):
        ...
        headers = list(rows[0].keys()) if rows else [dataset.get("district_col", "district"), dataset.get("value_col", "value")]
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=headers)
            writer.writeheader()
            writer.writerows(rows)

    output_png = os.path.join(rdir, f"{safe_slug(metadata.get('export_filename') or metadata.get('title'))}.png")
    state = {
        ...
        "dataset": {
            ...
            "source": dataset.get("source", "uploaded_csv" if source_csv else "atlas_internal_baseline"),
        },
        "metadata": metadata,
        "colormap": colormap,
...
def finalize_map_state(state: dict, map_id: str) -> dict:
    ...
    snapshot = {
        ...
        "execution_metadata": state.get("metadata", {}),
        "dataset": state.get("dataset", {}),
        "csv_snapshot": hist_csv,
        "map_snapshot": hist_png,
        "output_image": map_copy,
    ...
    return {
        ...
        "map_url": f"/storage/maps/{os.path.basename(map_copy)}?v={map_id}",
    }
