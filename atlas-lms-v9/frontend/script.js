...
async function sendMsg() {
  ...
  // Allow send if there's a message OR an attachment
  ...
  try {
    ...
    if (data.map_url) {
      ...
      hideDatasetBadge();   // clear "Map it" badge after map generated
    } else if (data.error) {
...
async function handleAttachment(event) {
  ...
  // Show uploading state in badge
  _showAttachBadge("⏳", file.name, false);
  ...
  try {
    ...
    // Store reference — will be sent WITH next message
    activeAttachment = {
      path: data.path,
      type: data.file_type,
      name: data.name,
    ...
    _showAttachBadge(ATTACH_ICONS[data.file_type] || "📎", file.name, true);
...
function _showAttachBadge(icon, name, ready) {
...
async function _uploadFileAsAttachment(file) {
  const ALLOWED_EXTS = [
    ...
    ".pdf",".xlsx",".xls",".docx",".doc",".txt",".md",".pptx"
  ];
...
// Show user message with file thumbnail in the chat bubble
function _appendUserMsgWithAttachment(msg, attachment) {
  const list = document.getElementById("messages");
  const div  = document.createElement("div");
  div.className = "msg user";

  const attachHtml = attachment ? `
    <div class="attach-thumb">
      <span class="at-icon">${attachment.icon}</span>
      <span class="at-name">${attachment.name}</span>
    </div>` : "";
