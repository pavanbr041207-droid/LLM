...
def send():
    ...
    # ── Check if user is replying YES/NO to a pending permission request ────
    try:
        ...
        if pending and pending.get("type") == "map":
            ...
            if is_confirmation(msg_lower) or is_denial(msg_lower):
                ...
                if guard.get("allowed"):
                    # User said YES — re-run the map pipeline with original csv_path
                    colormap = sidebar_cmap or "Blues"
                    if csv_path and os.path.exists(csv_path):
                        return _pipeline_from_csv(csv_path, session_id + " YES", colormap,
                                                   session_id, session_name, project_id)
                    from services.dataset_factory import generate_dataset, DATA_NOT_AVAILABLE
                    dataset = generate_dataset(pending.get("params", {}).get("user_msg", user_msg))
                    if dataset.get("status") == "ok":
                        return _pipeline_from_dataset(dataset, user_msg, colormap,
                                                       session_id, session_name, project_id)
                    # Unknown topic — LLM search
                    topic, geography, metric_col = infer_topic_geography_metric(
                        pending.get("params", {}).get("user_msg", user_msg)
                    )
                    ...
                    if extraction["status"] == "no_data":
                        ...
                        _flush(session_id, session_name, project_id)
                        return jsonify({"reply": reply, "mode": "chat", "csv_path": None})
                    fresh_csv = _save_extracted_csv(extraction["csv_text"],
                    ...
                    return _pipeline_from_csv(fresh_csv, user_msg, colormap,
                                               session_id, session_name, project_id,
    ...
    if is_map_request(user_msg, csv_path):
        ...
        # ── PERMISSION GATE: ask YES/NO before any backend execution ─────────
        try:
            from middleware.execution_guard import check_execution_guard
            guard = check_execution_guard(
                user_msg, session_id,
                {"is_map": True, "needs_execution": True},
                {"user_msg": user_msg}  # store original msg for YES re-trigger
            )
            if guard.get("denied"):
                reply = guard["reply"]
                _add(session_id, "assistant", reply)
            ...
            # guard["allowed"] == True → fall through to generation
        except Exception:
            pass  # if guard fails, allow generation to proceed
