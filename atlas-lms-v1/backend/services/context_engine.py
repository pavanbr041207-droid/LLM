...
def detect_intent(message: str) -> dict:
    ...
    """
    msg = message.lower()
    map_intent = _is_map_intent(msg)
    return {
        "has_reference":   any(re.search(p, msg) for p in REFERENCE_PATTERNS),
        "needs_execution": map_intent or any(re.search(p, msg) for p in EXECUTION_PATTERNS if "map" not in p),
        "is_update":       any(re.search(p, msg) for p in UPDATE_PATTERNS),
        "is_map":          map_intent,
        "is_dataset_request": _is_dataset_request(msg),
...
def _is_map_intent(msg: str) -> bool:
    visual_terms = ["choropleth", "map", "heat map", "heatmap", "visualize", "plot"]
    action_terms = ["generate", "create", "make", "draw", "render", "show"]
    return any(k in msg for k in ["choropleth", "heat map", "heatmap"]) or (
        any(a in msg for a in action_terms) and any(v in msg for v in visual_terms)
    )


def _is_dataset_request(msg: str) -> bool:
    data_terms = ["data", "dataset", "table", "values", "statistics"]
    return any(t in msg for t in data_terms) and not _is_map_intent(msg)
...
def build_context_prompt(user_msg: str, session_id: str,
    ...
    # Map execution must not auto-resolve a previous CSV. A fresh request state
    # is created by chat_routes for every map request.
